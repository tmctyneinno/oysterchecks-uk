...
use GuzzleHttp\ClientInterface;ClientInterface
...
class ApplicantController extends Controller
{
    ...
    private function sendToApi($request)
    {
     ...
     protected function createSignature(  $request, int $ts): string
     {


        $hash = hash_hmac('sha256', $ts . strtoupper($request->getMethod()) . $request->getUri() . $request->getBody(), "kQYTcpGXGoQmdcY73Hr6UJOl0QjEoFJ1");
        dd($hash);
     }
